import { BehaviorKind } from "../project";

/**
 * Beginner-friendly documentation for Blueprint components, shown in the
 * Documentation modal's Components tab. The PARAMETER ROWS are pulled live from
 * `BEHAVIOR_PARAMS` (behaviorMeta.ts) so the doc can never drift from the real
 * inspector fields — this file only supplies the plain-English prose. A param
 * with no entry in `params` falls back to its inspector label, so the tab still
 * renders every field even before its description is written.
 */
export interface ComponentDoc {
  /** Friendly display name (the BehaviorKind is shown as the code tag). */
  title: string;
  /** Section the component is grouped under in the docs. */
  group: string;
  /** One-liner: what this component IS, in plain words. */
  blurb: string;
  /** When you'd reach for it. */
  when?: string;
  /** key → plain-English "what this field does + an example". */
  params: Record<string, string>;
  /** Optional highlighted note. */
  tip?: string;
}

export const COMPONENT_GROUPS = [
  "Movement", "Visuals", "Physics", "Camera", "Combat & RPG", "AI", "UI", "Tilemap", "Misc",
] as const;

export const COMPONENT_DOCS: Partial<Record<BehaviorKind, ComponentDoc>> = {
  CharacterMovement: {
    title: "Character Movement",
    group: "Movement",
    blurb: "The full platformer controller — left/right running, gravity, jumping, dashing and wall-sliding, all driven by named Input Actions.",
    when: "Add it to a side-on player or platformer enemy. Don't combine it with Topdown Movement — pick one mover.",
    tip: "All these values can also be changed live during play with the CM Set… nodes.",
    params: {
      maxSpeed: "Top running speed in pixels/second. Higher = faster. 220 is a typical run.",
      acceleration: "How fast it speeds UP to max. High = snappy, low = a slippery start. 1500 ≈ responsive.",
      deceleration: "How fast it slows DOWN when you let go. High = stops on a dime, low = ice-skating.",
      airControl: "How much steering you have in mid-air (0..1). 1 = full control, 0 = committed jump with no air steering.",
      gravity: "How hard it's pulled down each second. Higher = heavier, falls faster. 800 ≈ normal.",
      gravityAngle: "Which way 'down' is, in degrees. 90 = straight down (normal). Change it for flipped / wall gravity.",
      maxFallSpeed: "Fastest it can fall (terminal velocity). Caps how fast gravity drags it. 600 keeps falls controllable.",
      ceilingMode: "What happens when it bonks a ceiling mid-jump. 0 = just stop rising (normal). Advanced.",
      mirrorMode: "Auto-flip the sprite to face the way it moves. 0 = off, 1 = flip by movement, 2 = flip by input.",
      scaleMirror: "When flipping to face the other way, smoothly turn instead of snapping instantly.",
      scaleMirrorTime: "How long that smooth flip takes, in seconds. 0.15 = a quick turn.",
      leftAction: "The named Input Action that moves left (set up in Input Actions). Rebinding keys there never breaks this.",
      rightAction: "The named Input Action that moves right.",
      jumpAction: "The named Input Action that jumps.",
      dashAction: "The named Input Action that dashes.",
      leftEventTrigger: "Optional: a custom event to fire when 'left' is pressed — bolt extra logic on. Leave empty if unused.",
      rightEventTrigger: "Optional custom event fired when 'right' is pressed.",
      jumpEventTrigger: "Optional custom event fired when 'jump' is pressed.",
      dashEventTrigger: "Optional custom event fired when 'dash' is pressed.",
      leftCustomFn: "Advanced internal hook. Leave empty.",
      rightCustomFn: "Advanced internal hook. Leave empty.",
      jumpCustomFn: "Advanced internal hook. Leave empty.",
      dashCustomFn: "Advanced internal hook. Leave empty.",
      wallSlideCustomFn: "Advanced internal hook. Leave empty.",
      wallJumpCustomFn: "Advanced internal hook. Leave empty.",
      jumpStrength: "How hard the jump launches upward. Higher = jumps higher. 460 ≈ a medium jump.",
      multiJump: "How many jumps before touching the ground again. 1 = single, 2 = double jump.",
      coyoteEnabled: "Coyote time — let the player still jump for a split second AFTER walking off a ledge. Feels forgiving.",
      coyoteTime: "How long that after-ledge grace lasts, in seconds. 0.1 = a tenth of a second.",
      bufferEnabled: "Jump buffer — if you press jump just BEFORE landing, it still jumps the instant you land.",
      bufferTime: "How early a pre-press still counts, in seconds. 0.1 default.",
      varHeightEnabled: "Variable jump height — tap = short hop, hold = full jump. Off by default.",
      varHeightCutoff: "(Variable height) when you release early, rising speed is clamped to this. More negative = taller minimum hop.",
      jumpSustainEnabled: "Let holding jump keep adding lift for a moment (a floatier rise). Off by default.",
      jumpSustainTime: "How long holding jump keeps boosting, in seconds.",
      dashEnabled: "Turn the dash ability on or off.",
      dashSpeed: "How fast the dash flings you, px/sec. 600 = a quick burst.",
      dashStartDelay: "Wind-up before the dash actually moves, in seconds. 0 = instant.",
      dashDuration: "How long the dash lasts, in seconds. 0.15 = a short blink.",
      dashCooldown: "How long before you can dash again, in seconds.",
      dashWallBlock: "On = a dash stops at walls instead of clipping through them.",
      wallEnabled: "Turn wall-sliding and wall-jumping on.",
      wallJumpAllowed: "Allow jumping OFF a wall while sliding on it.",
      wallJumpStrength: "Upward power of a wall jump.",
      wallJumpKickX: "Sideways push-off power of a wall jump (kicks you away from the wall).",
      wallSlideSpeed: "How fast you slide DOWN a wall while clinging. Lower = stickier.",
      wallSlideRequiresInput: "On = you must hold toward the wall to cling. Off = clings automatically.",
      wallSlideOnContact: "On = start sliding the moment you touch a wall while airborne.",
    },
  },

  TopdownMovement: {
    title: "Topdown Movement",
    group: "Movement",
    blurb: "8-direction, gravity-free movement for top-down / RPG / flying objects. Moves on all four axes via named Input Actions.",
    when: "Add it to a top-down player or creature. Don't combine it with Character Movement.",
    params: {
      maxSpeed: "Top move speed, px/sec. (If an AI Brain drives this NPC, the brain sets it.)",
      acceleration: "How fast it speeds up to max.",
      deceleration: "How fast it slows to a stop when you let go.",
      upAction: "The Input Action that moves up.",
      downAction: "The Input Action that moves down.",
      leftAction: "The Input Action that moves left.",
      rightAction: "The Input Action that moves right.",
      upEventTrigger: "Optional custom event fired when 'up' is pressed.",
      downEventTrigger: "Optional custom event fired when 'down' is pressed.",
      leftEventTrigger: "Optional custom event fired when 'left' is pressed.",
      rightEventTrigger: "Optional custom event fired when 'right' is pressed.",
      mirrorMode: "Auto-flip the sprite: 0 = off, 1 = by movement direction, 2 = by input.",
      scaleMirror: "Smoothly turn when flipping facing, instead of snapping.",
      scaleMirrorTime: "How long the smooth flip takes, in seconds.",
      ignoreInput: "Lock the player's input so momentum just coasts — for cutscenes or knockback.",
      slideAssist: "Slide along a diagonal wall instead of sticking to it.",
    },
  },

  MoveTo: {
    title: "Move To",
    group: "Movement",
    blurb: "A simple 'walk toward something' steering driver — a point, a specific object, the nearest tagged object, or a heading. Also runs nav-mesh patrols.",
    when: "Use it for NPCs that wander, chase, or follow a path. The AI Brain can also drive this for you.",
    params: {
      mode: "How it picks where to go. Position = a fixed X,Y. Object = one specific sprite. Tag = the nearest sprite with a tag. Angle = just head in a straight line.",
      targetX: "(Position mode) the X to walk to.",
      targetY: "(Position mode) the Y to walk to.",
      targetUid: "(Object mode) the specific sprite to follow. Usually set by logic, not by hand.",
      targetTag: "(Tag mode) walk to the nearest sprite carrying this tag, e.g. 'player'.",
      angleDeg: "(Angle mode) heading to move along, in degrees. 0 = right, 90 = down.",
      speed: "Move speed in px/sec. (If an AI Brain drives this NPC, the brain sets it.)",
      stopRadius: "How close counts as 'arrived', in px. Bigger = stops further away.",
      retargetEverySec: "(Tag mode) how often to re-scan for a new nearest target, in seconds.",
      arrivalSignal: "Optional signal fired when it reaches the target — catch it in the Logic Sheet.",
      mirror: "Flip the sprite to face its walking direction.",
      usePhysics: "On = moves the physics body so walls stop it. Off = slides straight to the target each frame (cheaper, ignores walls).",
      enabled: "Turn the movement on/off — e.g. freeze it during a cutscene.",
      separationDist: "Keep this many px of space from same-tag neighbors (0 = off). Stops a crowd stacking into one pile.",
      separationTag: "Which neighbors to keep apart from (empty = the host's own first tag).",
      separationStrength: "Leftover/unused — ignore it.",
      separationMode: "How crowding is resolved. Off = overlap freely. Velocity = gentle swerve. Push = hard min-gap (Vampire-Survivors style).",
      separationAvoid: "Not built yet — has no effect (see the grayed note in the inspector).",
    },
  },

  SpriteRenderer: {
    title: "Sprite Renderer",
    group: "Visuals",
    blurb: "Draws an animated sprite on the object. This is what makes a Blueprint actually look like something instead of a colored box.",
    when: "Add it to anything that should show artwork. Pick a sprite you made in the Sprite editor.",
    params: {
      spriteId: "Which sprite asset to draw. Pick one from your project's sprites.",
      currentAnimation: "Which animation of that sprite plays by default (e.g. 'idle').",
      playing: "On = the animation plays. Off = freeze on a single frame.",
      speed: "Playback speed multiplier. 1 = normal, 2 = double speed, 0.5 = half.",
      frame: "Which frame to show when not playing (a still pose).",
      useFrameCollider: "Advanced: use per-frame collision shapes baked into the sprite instead of the box Collider.",
      solid: "(Frame collider) whether those per-frame shapes physically block other objects.",
      collideFilterMode: "(Frame collider) include only — or exclude — the tags listed below.",
      collideFilterTags: "(Frame collider) which tags the shapes collide with.",
    },
  },

  Collider: {
    title: "Collider",
    group: "Physics",
    blurb: "Sets the invisible physics box that actually bumps into walls and other objects — separate from how big the picture looks.",
    when: "Add it when the default body size (matching the sprite box) is wrong, or you want a trigger/sensor.",
    params: {
      width: "Width of the physics box in px. This is what collides — not the artwork.",
      height: "Height of the physics box in px.",
      offsetX: "Nudge the box left/right from the sprite center (e.g. line it up with feet).",
      offsetY: "Nudge the box up/down from the sprite center.",
      collideWorldBounds: "Keep this object inside the scene edges — it can't walk off the map.",
      passThrough: "On = others can overlap it (a trigger/sensor zone). Off = it's solid and blocks.",
      debugDraw: "Draw the box during Play so you can see it. Turn off for release.",
    },
  },

  Solid: {
    title: "Solid",
    group: "Physics",
    blurb: "Marks the object as immovable world geometry — walls, floors, platforms that other bodies collide against.",
    when: "Add it to static level pieces that should block the player and enemies.",
    params: {
      debugDraw: "Draw the solid's outline during Play to debug placement.",
    },
  },

  JumpThru: {
    title: "Jump-Through Platform",
    group: "Physics",
    blurb: "A one-way platform: bodies pass UP through it from below, then land on TOP. Classic for floating platforms.",
    when: "Add it to ledges you want to jump up onto but not bonk your head on. No settings — just attach it.",
    params: {},
  },

  Tracer: {
    title: "Tracer",
    group: "Combat & RPG",
    blurb: "A reach that detects (and optionally damages) other objects — a line for line-of-sight or bullets, or a box for melee swings. Also used as an AI's 'eyes'.",
    when: "Add it for melee hitboxes, sight checks, or any 'is something in front of me?' test. You can attach several (named).",
    params: {
      name: "A label for this tracer so logic and other components can target it (e.g. an AI's sight vs attack tracer).",
      shape: "Line = a thin ray (line-of-sight, bullets). Box = a rectangle area (melee swing).",
      distance: "How far the tracer reaches, in px.",
      angle: "Direction it points, in degrees. 0 = straight forward (the way the sprite faces).",
      pivotSource: "Where it starts from: the sprite center, a frame hotspot, a named image point, or a weapon's image point.",
      imagePointName: "Which named point on the sprite to start from (image-point modes).",
      weaponSlotName: "Which weapon slot's point to start from (weapon mode; empty = the first slot).",
      pivotX: "Nudge the start point left/right by this many px (mirrors with facing).",
      pivotY: "Nudge the start point up/down by this many px.",
      boxThickness: "(Box shape) how thick the box is, in px.",
      tagFilter: "Only detect sprites with these tags (comma list; empty = anything).",
      damage: "Damage dealt to whatever it hits. 0 = it's just a sensor, no damage.",
      multiHit: "On = hit everyone in range at once (cleave). Off = only the first target.",
      knockbackX: "Forward push dealt to what it hits.",
      knockbackY: "Vertical push dealt to what it hits.",
      triggerMode: "When it fires: Interval (on a timer / every frame) or On Signal (you fire it from logic).",
      intervalSec: "(Interval) seconds between checks; 0 = every frame.",
      triggerSignal: "(Signal) the signal name that makes it fire.",
      signalCount: "(Signal) how many hits per fire.",
      signalLoop: "(Signal) keep firing after the first burst.",
      signalLifetimeSec: "(Signal) how long a hit stays active; 0 = forever.",
      fireAnim: "Only allow it to fire during this animation (empty = any). e.g. 'attack'.",
      fireFrame: "Only fire on this frame index (-1 = any) — e.g. the exact swing impact frame.",
      debugDraw: "Draw the tracer during Play so you can see its reach.",
    },
  },

  Damageable: {
    title: "Damageable",
    group: "Combat & RPG",
    blurb: "Gives the object HP so it can take damage, get knocked back, and die. Reads/writes HP from a Blueprint variable.",
    when: "Add it to anything that can be hurt — players, enemies, breakable crates.",
    tip: "A hit fires exactly ONE signal: a clean hit → On Damage Taken; a full block (Guard Damage × = 0) → On Blocked; a partial block (0 < Guard Damage × < 1) → On Partial Block. They never overlap, so a hurt sound on On Damage Taken won't play on a block.",
    params: {
      hpVar: "Which BP variable holds current HP. The number itself lives in the Variables section.",
      maxHpVar: "Which BP variable holds max HP.",
      iframeSec: "Invincibility time right after a hit, in seconds — stops instant multi-hits.",
      hitstunSec: "How long it's stunned (can't act) after a hit.",
      knockbackMultiplier: "Three knockback scales, one per hit outcome: 'for damage' = a clean hit, 'for block' = a full block, 'for partial' = a partial block. 1 = normal, 2 = flies twice as far, 0 = immovable.",
      destroyOnDeath: "On = the object is removed when HP hits 0.",
      deathDestroyDelay: "Wait this long after death before removing it (lets a death animation play).",
      allowHealing: "Allow HP to go back up (heals, potions).",
      attackable: "On = AI enemies will target and attack it. Off = they ignore it.",
      blockStates: "Animator state names that count as blocking (e.g. 'block,parry') — damage is reduced while in them.",
      guardMultiplier: "Damage taken while blocking. 0 = full block → On Blocked (no damage). 0.5 = half → On Partial Block. 1 = no block → On Damage Taken as normal.",
      guarding: "A manual block on/off flag you can drive from logic.",
    },
  },

  Inventory: {
    title: "Inventory",
    group: "Combat & RPG",
    blurb: "A bag of item slots the object can carry. Items are project assets added at runtime via Give Item / pickups.",
    when: "Add it to the player (and to chests / shops / NPCs that hold items).",
    tip: "Give the PLAYER a Persist Key so its bag survives scene changes; leave it EMPTY on chests so each has its own.",
    params: {
      capacity: "How many item slots the bag has.",
      persistKey: "Set a key (e.g. 'player') so this bag carries across scenes and feeds the HUD. Leave EMPTY for a scene-local, independent bag.",
    },
  },

  AIBrain: {
    title: "AI Brain",
    group: "AI",
    blurb: "A ready-made enemy brain: it senses a target, chases, attacks, and flees — all automatically. Drives the movement component for you.",
    when: "Add it to an enemy that should act on its own. Use either AI Brain OR drive the NPC by hand from the Logic Sheet, not both.",
    tip: "It needs a Tracer named to match Sight/Attack Tracer Name, and a movement component to drive.",
    params: {
      autoBrain: "On = use the built-in brain (sense → chase → attack). Off = you drive the NPC yourself from the Logic Sheet.",
      state: "Which AI state it starts in (idle, alert, chase, attack, flee, search).",
      targetTag: "Who it hunts — the tag of its enemy, e.g. 'player'.",
      sightTracerName: "Name of the Tracer used as its eyes (line-of-sight). Must match a Tracer on this BP.",
      attackTracerName: "Name of the Tracer used as its attack reach / hitbox.",
      attackRange: "How close (px) the target must be before it attacks.",
      loseSightAfterSec: "Keep chasing this long after losing sight of the target, then give up.",
      attackDurationSec: ">0 = one distinct swing per attack with a rest between. 0 = continuous attacking.",
      attackCooldownSec: "Seconds between attack swings.",
      attackRestState: "Which state to show between swings (e.g. alert).",
      interruptAttackOnHit: "On = getting hit cancels its attack. Off = super armor (attack continues through hits).",
      chaseSpeed: "Move speed while chasing.",
      patrolSpeed: "Move speed while patrolling / wandering.",
      fleeOnDamage: "On = runs away when it gets hurt.",
      fleeDurationSec: "How long it flees before recovering.",
      hearSignals: "Signal names it 'hears' and reacts to (comma list) — e.g. noises that alert it.",
      patrolMode: "What it does with no target: None = stand still, Bounce on walls = wander and turn at walls.",
      autoFaceTarget: "Auto-turn to face its target.",
      disableWhenDead: "Stop thinking once HP ≤ 0.",
      separationDist: "Keep this much space from fellow enemies (0 = off) so they don't stack into one blob.",
      separationTag: "Which enemies to keep apart from.",
      separationMode: "How crowding is resolved (when the mover is MoveTo). Off / Velocity swerve / Push apart.",
      aiTickRate: "How often it 'thinks', in frames. 1 = every frame (60Hz, smartest, costliest), 3 = 20Hz (default), 6 = 10Hz (cheap for big crowds).",
      mover: "Which movement component the brain drives: Auto, Character Movement (platformer), Topdown (4-way), or Move To (steering/chase/patrol).",
    },
  },

  Text: {
    title: "Text",
    group: "Visuals",
    blurb: "Draws a text label on the object — names, damage numbers, signs. Supports {var:hp}-style placeholders that fill in live values.",
    params: {
      name: "A label for this text component, so logic can target it when a Blueprint has more than one.",
      content: "The words to show. Use {var:hp} / {self.x} placeholders and they fill in live each frame.",
      fontFamily: "Which font to use (a web font like Arial, or one you uploaded).",
      fontSize: "Text height in pixels.",
      color: "Text color as a hex number (0xRRGGBB) — e.g. 0xffffff is white.",
      bold: "Bold the text.",
      italic: "Italicize the text.",
      align: "Horizontal alignment of multi-line text — left, center, or right.",
      vAlign: "Vertical alignment — top, middle, or bottom.",
      wrapWidth: "Wrap onto new lines past this many px wide. 0 = never wrap.",
      visible: "Show or hide the text.",
      alpha: "Opacity, 0 (invisible) to 1 (solid).",
      offsetX: "Nudge the text left/right from the object's center, in px.",
      offsetY: "Nudge the text up/down from the object's center, in px.",
    },
  },
  TiledBackground: {
    title: "Tiled Background",
    group: "Visuals",
    blurb: "A repeating, scrolling background image — skies, clouds, parallax layers that drift behind the action.",
    params: {
      spriteId: "The image to repeat across the background.",
      currentAnimation: "Which animation of that sprite to use (for an animated background).",
      playing: "On = animate the tiles. Off = hold one still frame.",
      startFrame: "(Not playing) which frame to show.",
      mode: "Follow Camera = parallax-scrolls with the camera. Auto Scroll = drifts on its own at a set speed.",
      parallaxFactorX: "(Follow Camera) how much it moves with the camera horizontally. 0 = locked to the screen (very far), 1 = moves with the world, 0.05 = distant backdrop.",
      parallaxFactorY: "(Follow Camera) same, vertically.",
      scrollSpeedX: "(Auto Scroll) horizontal drift, px/sec.",
      scrollSpeedY: "(Auto Scroll) vertical drift, px/sec.",
      width: "Width of the background area in px. 0 = fill the viewport.",
      height: "Height of the background area in px. 0 = fill the viewport.",
      flipX: "Mirror each tile horizontally.",
      flipY: "Mirror each tile vertically.",
      tileX: "Repeat the image across X. Turn off for a single copy (no horizontal loop).",
      tileY: "Repeat the image across Y. Turn off for a single strip — e.g. a horizon band.",
      enabled: "Turn the background on or off.",
    },
  },
  SquashStretch: {
    title: "Squash & Stretch",
    group: "Visuals",
    blurb: "Cartoon squash-and-stretch wobble — adds juice to jumps, landings, and hits. Trigger it with the Play Squash Stretch node.",
    params: {
      intensity: "How much it deforms, 0..1. Higher = more exaggerated squash/stretch.",
      duration: "How long one squash-and-return takes, in seconds.",
      easing: "The motion curve — e.g. Back Out overshoots, Bounce Out bounces, Quad Out is a smooth settle.",
      emitOnEnd: "Fire the OnSquashStretchEnd signal when it finishes, so you can chain more logic.",
    },
  },
  ParticleEmitter: {
    title: "Particle Emitter",
    group: "Visuals",
    blurb: "Spits out particles — smoke, sparks, blood, dust. Runs continuously or fires one-off bursts on command.",
    params: {
      name: "A label so logic can target this emitter (a Blueprint can have several).",
      mode: "Continuous = emits steadily at a rate. Burst = fires a one-off puff when you trigger it.",
      rate: "(Continuous) particles spawned per second.",
      burstCount: "(Burst) how many particles per burst.",
      maxParticles: "A soft cap on how many particles can be alive at once.",
      enabled: "Emit On Start — begin emitting the moment it spawns.",
      spriteId: "The image each particle uses.",
      delay: "Wait this long before the first emission, in seconds.",
      pivotSource: "Where particles spawn from — the object's center, or a named image point on its sprite.",
      imagePointName: "(Image Point) which named point to spawn from.",
      spawnJitterX: "Randomly offset each spawn left/right by up to ± this many px.",
      spawnJitterY: "Randomly offset each spawn up/down by up to ± this many px.",
      offsetX: "Shift the spawn point left/right from the pivot, in px.",
      offsetY: "Shift the spawn point up/down from the pivot, in px.",
      renderOrder: "Draw particles in front of or behind the object's sprite.",
      lifetime: "How long each particle lives, in seconds.",
      lifetimeJitter: "Random ± variation on lifetime, so they don't all die at once.",
      speed: "Initial particle speed, px/sec.",
      speedJitter: "Random ± variation on speed.",
      angleMin: "Start of the emission direction range, in degrees (-90 = straight up).",
      angleMax: "End of the emission direction range. Equal to min = a single direction.",
      gravityX: "Constant sideways pull on particles, px/sec².",
      gravityY: "Constant vertical pull on particles, px/sec² (positive = down).",
      friction: "Slows particles over their life, 0 (none) .. 1 (heavy drag).",
      rotationStart: "Particle spin angle at birth, in degrees.",
      rotationEnd: "Particle spin angle at death — the particle rotates from start to end.",
      rotationJitter: "Random ± variation on rotation.",
      scaleStart: "Particle size at birth (1 = native).",
      scaleEnd: "Particle size at death — e.g. 1 → 0 shrinks away.",
      alphaStart: "Opacity at birth, 0..1.",
      alphaEnd: "Opacity at death — e.g. 1 → 0 fades out.",
      tintStart: "Color tint at birth (0xRRGGBB).",
      tintEnd: "Color tint at death — particles lerp from start to end.",
      blendMode: "How particles blend — Normal, Additive (glowy, for fire/magic), or Multiply (darken).",
      frameMode: "Use the sprite's first frame, or a random frame, for each particle.",
      frameIndices: "Limit to specific frame numbers (0-based, comma-separated).",
    },
  },
  VisionMask: {
    title: "Vision Mask",
    group: "Visuals",
    blurb: "A darkness/fog cutout around the object — a flashlight cone, fog of war, or spotlight effect.",
    params: {
      radius: "Size of the clear (visible) area around the object, in px.",
      featherPx: "Soft fade at the edge of the clear area, in px. 0 = a hard edge.",
      cutoutOpacity: "How dark the masked area is. 0 = fully cut (dark), 1 = no darkening.",
      maskSpriteId: "Use a sprite's shape (its alpha) as the cutout instead of a plain circle — e.g. a cone for a flashlight.",
      maskSpriteMode: "Static (one frame) or Animation (cycling) for the mask shape.",
      maskAnimation: "(Animation) which animation to cycle for the shape.",
      maskFrame: "(Static) which frame of the mask sprite to use.",
      maskMirror: "Flip the mask shape with the object's facing.",
      centerOffsetX: "Move the clear area's center left/right from the object, in px.",
      centerOffsetY: "Move the clear area's center up/down from the object, in px.",
      cutoutLayers: "Which scene layers the darkness covers.",
      excludeTags: "Objects carrying these tags are never darkened.",
      invert: "Flip it into a spotlight — darkness INSIDE the shape instead of outside.",
      enabled: "Turn the mask on or off.",
    },
  },
  Dismemberment: {
    title: "Dismemberment",
    group: "Visuals",
    blurb: "Lets parts of the sprite detach or break off — gore, shattering armor, breakable pieces.",
    params: {
      refAnimation: "The pose the pieces are cut from (empty = the object's current pose).",
      refFrame: "Which frame of that pose to slice.",
      launch: "How pieces fly off: Burst (outward), Drop (down + spread), Directional (a set angle), or Random (chaotic).",
      angleDeg: "(Directional) the angle pieces fly toward (270 = up).",
      spreadDeg: "How wide the spray cone is, in degrees.",
      speedMin: "Each piece flies at a random speed — this is the low end, px/sec.",
      speedMax: "The high end of the random launch speed, px/sec.",
      spinMin: "Each piece spins at a random rate — low end, deg/sec.",
      spinMax: "High end of the random spin, deg/sec.",
      gravity: "Downward pull on the pieces, px/sec².",
      bounce: "How bouncy pieces are off the world, 0 (none) .. 1 (very).",
      cleanup: "When pieces disappear — Time (fade after a lifetime), Offscreen (when off-camera), or Pool (cap the count, oldest fade first).",
      lifetimeSec: "(Time) how long pieces last before fading.",
      fadeSec: "How long the fade-out takes, in seconds.",
      maxGibs: "(Pool) the most pieces kept alive at once.",
      collideWorld: "Pieces collide with solid world geometry.",
      hideHost: "Hide the original object when it's dismembered.",
      fireSignal: "Auto-trigger the dismemberment when this signal fires.",
    },
  },
  Projectile: {
    title: "Projectile",
    group: "Combat & RPG",
    blurb: "Makes the object fly like a bullet or arrow — straight-line or homing — dealing damage and dying on hit.",
    params: {
      mode: "Straight-line = flies in a fixed direction. Homing = curves toward a target and keeps tracking it. Aimed = snapshots the target's position when fired and flies straight there with NO tracking — the player can side-step it (classic top-down shot). Aimed + Homing both use Target Tags to pick what to aim at.",
      speed: "Flight speed, px/sec.",
      lifetime: "Auto-destroy after this many seconds. 0 = lives forever.",
      gravityX: "Constant sideways pull, px/sec².",
      gravityY: "Constant vertical pull, px/sec² — add some for an arcing lob.",
      targetTags: "Which tags it can hit (and, in Homing mode, steer toward).",
      hitSignal: "Signal fired when it hits a valid target.",
      destroyOnHit: "Remove the projectile the moment it hits something.",
      collideTiles: "Also collide with tilemap tiles (walls).",
      tileHitSignal: "(Collide with tiles) signal fired when it hits a tile.",
      rotateToVelocity: "Auto-rotate the sprite to point the way it's traveling.",
      damage: "Damage dealt on hit. 0 = no damage (a sensor/visual only).",
      knockbackX: "Sideways push applied to what it hits.",
      knockbackY: "Vertical push applied to what it hits.",
      hitboxW: "Custom hit-area width. 0 = use the body size.",
      hitboxH: "Custom hit-area height. 0 = use the body size.",
      hitboxOffsetX: "Nudge the hit area left/right from center.",
      hitboxOffsetY: "Nudge the hit area up/down from center.",
      debugDraw: "Draw the hitbox during Play to see it.",
      homingTurnRate: "(Homing) how fast it can turn toward the target, deg/sec. Higher = sharper tracking.",
    },
  },
  WeaponSlot: {
    title: "Weapon Slot",
    group: "Combat & RPG",
    blurb: "Pins a weapon sprite to a point on the character (like a hand) so it moves and flips with them.",
    params: {
      name: "Slot label (e.g. RightHand) — used by the Equip Weapon action and weapon:NAME.field reads.",
      spriteId: "The weapon sprite to show. Empty = unequipped.",
      currentAnimation: "Which weapon animation to play (swing, idle…).",
      imagePoint: "The named point on the wielder's sprite to attach the weapon to — the hand.",
      offsetX: "Nudge the weapon left/right from that point, in px (mirrors with facing).",
      offsetY: "Nudge the weapon up/down from that point, in px.",
      angleOffset: "Rotate the weapon, in degrees.",
      scaleX: "Weapon width (1 = the sprite's native size).",
      scaleY: "Weapon height (1 = native).",
      followFacing: "Mirror the weapon when the character flips direction.",
      renderAbove: "Draw the weapon in front of the character (off = behind).",
      visible: "Show or hide the weapon.",
      playing: "Advance the weapon's animation frames.",
      startFrame: "(Not playing) which frame to hold.",
      speed: "Animation playback speed multiplier (1 = normal, 2 = double).",
    },
  },
  PhaseManager: {
    title: "Phase Manager",
    group: "AI",
    blurb: "Boss phases — splits a fight into stages at HP thresholds (e.g. 66% and 33%) you can react to in logic.",
    params: {
      thresholdsPct: "HP percentages that start each new phase, comma-separated. e.g. 66,33 makes three phases.",
      currentPhase: "Which phase it starts in (0 = the first).",
      phaseVar: "A variable that mirrors the current phase number — read it in logic or animation conditions.",
      invulnOnTransitionSec: "Brief invincibility while switching phases, in seconds (a fair pause for the cutscene/animation).",
    },
  },
  Widget: {
    title: "Widget",
    group: "UI",
    blurb: "Pins a UI widget — like a healthbar or name label — above this object in the world. Each instance shows its own values.",
    params: {
      widgetId: "Which UI widget to pin above this object (e.g. a healthbar you made).",
      linkedVar: "A Blueprint variable that drives the widget's value/text each frame — e.g. hp → a healthbar fill.",
      offsetX: "Position the widget left/right of the object, in px.",
      offsetY: "Position the widget up/down — e.g. -40 floats it above the head.",
      hideWhenDead: "Auto-hide the widget when the object's Damageable is dead.",
    },
  },
  SmartTween: {
    title: "Smart Tween", group: "Misc", params: {},
    blurb: "Keyframe-animate an object's transform and color over time — opening doors, moving platforms, bobbing pickups, hit flashes. You author named animations in a custom card (not flat fields), then play them with the Play Smart Tween node.",
    when: "Use for scripted, replayable motion / color tweens. For character animation states, use the State Machine instead.",
    tip: "Each keyframe has a TIME plus channels: position offset (X/Y), scale, opacity, rotation, a per-keyframe Signal, and a Tint. Tint has two modes (set per animation): Tint = multiply (colored glows; white = no change) and Fill = solid silhouette (white = a full-white flash). Each keyframe's tint has an on/off checkbox — turn it OFF to END a flash. Loop / Play-on-start / Mirror-with-facing and the Target (host, or a specific component like a WeaponSlot) live on the animation row.",
  },
  TilemapRenderer: {
    title: "Tilemap Renderer", group: "Tilemap", params: {},
    blurb: "Draws a tilemap. Configured by the tilemap asset itself (in the Tilemap editor), not by per-object fields.",
  },
  Shadow: {
    title: "Shadow",
    group: "Visuals",
    blurb: "A soft blob shadow under the object — circle or rectangle, with feathered edges. Can snap onto the ground below, so a jumping character leaves its shadow on the floor.",
    when: "Add it to characters and props that should feel grounded. For platformer jumps, set Ground Tag(s) so the shadow stays on the ground while the body is airborne.",
    tip: "The offset is draggable as a gizmo in the Blueprint preview. The shadow scales with the object automatically.",
    params: {
      on: "Show or hide the shadow. Also toggleable at runtime with Set Component Param.",
      shape: "Circle (a squashed ellipse — the classic blob) or Rectangle.",
      width: "Shadow width in px.",
      height: "Shadow height in px — keep it smaller than width for a floor-hugging ellipse.",
      feather: "Edge softness, 0 (crisp) to 1 (very soft/blurred).",
      opacity: "How dark the shadow is, 0..1.",
      color: "Shadow color (hex). Usually black.",
      offsetX: "Nudge left/right from the object's origin, in px.",
      offsetY: "Nudge up/down — usually a few px down to sit at the feet.",
      groundTag: "Tags of ground objects (comma list). When set, the shadow SNAPS its Y onto the nearest tagged surface below — the platformer jump shadow. Empty = just follows the object.",
      maxDrop: "How far down (px) to search for a ground surface.",
      shrinkWithHeight: "Shrink + fade the shadow the higher the object is above the ground (classic jump feel). Needs Ground Tag(s).",
    },
  },
  LightSource: {
    title: "Light Source",
    group: "Visuals",
    blurb: "An atmospheric glow around the object — a baked additive light that renders above the ambient darkness. Lanterns, torches, magic auras.",
    when: "Add it to anything that should glow in a dark scene (pair with Set Ambient Light for the darkness itself).",
    tip: "The center offset is draggable as a gizmo in the Blueprint preview.",
    params: {
      on: "Turn the glow on or off.",
      color: "Glow color (hex) — warm orange for torches, pale blue for magic.",
      radius: "Glow size in px (the lit circle's radius).",
      intensity: "Brightness, 0..1.",
      edge: "Rim style: Smooth (soft falloff), Hard (crisp circle), Noisy or Wave (organic, flickery rims).",
      feather: "How soft the falloff is toward the rim.",
      edgeAmount: "How strong the Noisy/Wave rim distortion is.",
      offsetX: "Move the glow's center left/right from the object, in px.",
      offsetY: "Move the glow's center up/down, in px.",
      flicker: "Adds a candle-like brightness wobble, 0 (steady) to 1 (heavy flicker).",
      flickerSpeed: "How fast the flicker wobbles.",
    },
  },
  Outline: {
    title: "Outline",
    group: "Visuals",
    blurb: "A colored stroke around the object's sprite (8-direction offset copies of the art), with an optional additive glow. Selection highlights, interactables, rare-item shine.",
    when: "Add it to things the player should notice — an interactable NPC, a highlighted pickup. Toggle it from logic via Set Component Param.",
    tip: "It outlines the SpriteRenderer's art — the BP needs a visible sprite for the outline to trace.",
    params: {
      on: "Show or hide the outline.",
      color: "Stroke color (hex).",
      thickness: "Stroke thickness in px.",
      opacity: "Stroke opacity, 0..1.",
      pulse: "Pulse the thickness in and out — a breathing highlight.",
      pulseSpeed: "Pulse cycles per second.",
      glow: "Add a soft additive glow around the sprite (layered silhouette copies).",
      glowColor: "Glow color (hex).",
      glowSize: "How far the glow reaches beyond the sprite, in px.",
      glowOpacity: "Glow strength, 0..1.",
      feather: "Glow softness — how gradually it fades out.",
    },
  },
  Weather: {
    title: "Weather",
    group: "Visuals",
    blurb: "Drop-in rain or snow across the scene. Particles only exist inside the viewport, so it's cheap even on huge maps. Supports splashes, wind, sway, and shelter (no rain over roofs).",
    when: "Add ONE Weather object per scene (an invisible BP is fine). Tune shape/speed/angle for rain vs snow.",
    tip: "Two ways to shelter areas from the weather: tag moving objects (umbrellas, NPCs) and list the tag in Shelter Tags, or PAINT static areas in the Nav Mesh editor's ☂ tools — blue = fully dry, red = rain falls but no splashes.",
    params: {
      on: "Turn the weather on or off.",
      space: "Screen = a camera-fixed sheet (always around the player). World = particles live in world coordinates (parallax correct).",
      mode: "Top-down = drops land at varied heights for depth. Side-scroller = drops fall until they hit a Kill-Tag object, then splash.",
      killTags: "(Side-scroller) tags whose objects stop falling drops (ground, trees).",
      shelterTags: "Objects with these tags are FULLY DRY — no drops, no splashes over them (roofs, awnings). For static areas prefer the painted shelter mask.",
      shelterDrizzleTags: "Objects with these tags keep the rain falling but remove its splashes.",
      count: "How many particles (density).",
      speed: "Fall speed, px/sec.",
      speedJitter: "Random ± variation on speed, 0..1.",
      angle: "Fall direction in degrees from straight down (+ = leaning right).",
      wind: "Constant sideways drift, px/sec.",
      rotationSpeed: "Per-particle spin, deg/sec (for tumbling snowflakes). 0 = lines align to the fall.",
      shape: "Line (rain streaks), Circle (snow / dots), or Sprite (your own particle art).",
      spriteId: "(Sprite shape) which sprite each particle uses.",
      size: "Particle size in px.",
      sizeJitter: "Random ± variation on size, 0..1.",
      thickness: "(Line shape) streak thickness in px.",
      color: "Particle color (hex) for line/circle shapes.",
      alpha: "Particle opacity, 0..1.",
      sway: "Horizontal sway amplitude in px — gives snow its drift.",
      swaySpeed: "How fast the sway oscillates.",
      splash: "Spawn a splash where a drop lands (side-scroller kill or top-down landing).",
      splashType: "Simple = a small crown of pixels. Sprite = play a one-shot animation.",
      splashSprite: "(Sprite splash) the sprite asset to spawn.",
      splashAnim: "(Sprite splash) which animation to play once (empty = first).",
      splashScale: "(Sprite splash) scale of the splash sprite.",
    },
  },
};
