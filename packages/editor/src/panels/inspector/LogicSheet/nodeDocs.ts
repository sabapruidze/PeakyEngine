...
const FILL: Record<string, [string, string]> = {
  ...
  OnKeyHeldFor: ["Fires ONCE after a key has been held continuously for N seconds — a long-press / charge. Re-arms after you release.", "Hold Attack 1.5s → release a charged shot."],
  OnSeparate: ["Fires when this object STOPS overlapping a tagged object.", "Leave the trigger zone → hide the prompt."],
  OnOverlapForSeconds: ["Fires ONCE after this object has overlapped a tagged object continuously for N seconds — a proximity hold. Re-arms after separation.", "Stand in the healing pool 3s → restore health."],
  ...
  TweenVar: ["Smoothly ramps a VARIABLE from a start value to an end value over time, with easing — frame-rate independent. yoyo + repeat = a pulsing value.", "Tween 'charge' 0 → 100 over 2s → drive a UI bar."],
  TweenParam: ["Smoothly ramps a COMPONENT parameter (e.g. LightSource radius / intensity) from a start to an end value over time, with easing. yoyo + repeat = pulsing.", "Tween LightSource.radius 0 → 200 over 1.5s → light grows in."],
  ...
  GetOverlappingObject: ["Reads a field of what you're CURRENTLY overlapping — query any time (e.g. on a key press), no collision trigger needed.", "Press E → Get Overlapping name → Switch."],
  GetDistance: ["Distance in px from this object to a target: picked / nearest-tag / named-BP / mouse / point. Real distance, no AIBrain needed.", "Get Distance(tag 'enemy') < 150 → attack."],
  GetHoveredObject: ["Reads a field of the TOPMOST object under the cursor (name, tag, x, y, uid). Query any time.", "On Object Clicked → Get Hovered name → Switch."],
  OnObjectHovered: ["Fires ONCE when the cursor enters this object.", "Hover sprite → enable its outline."],
  OnObjectUnhovered: ["Fires ONCE when the cursor leaves this object.", "Unhover → disable its outline."],
  IsCursorOverObject: ["True every frame the cursor is over a matching object (by tag, or any if blank).", "Cursor over enemy → show tooltip."],
