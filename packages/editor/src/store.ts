...
interface EditorState {
  ...
  /** Add a new event row at the end of `parentId`'s children, or at top level if null. */
  addEvent: (bpId: string, parentId: string | null, kind: PeakyEventKind, pageId?: string) => string;
  /** Set the kind ("event" or "comment"). Switching to comment wipes content. */
  setEventKind: (bpId: string, eventId: string, kind: PeakyEventKind) => void;
  /** Set how the event's conditions are combined: AND (default) or OR. */
  setEventCombinator: (bpId: string, eventId: string, combinator: "AND" | "OR") => void;
  setEventCommentText: (bpId: string, eventId: string, text: string) => void;
  setEventDisabled: (bpId: string, eventId: string, disabled: boolean) => void;
  removeEvent: (bpId: string, eventId: string) => void;
  duplicateEvent: (bpId: string, eventId: string) => string;
  /** Move event up/down within its sibling list. */
  moveEventPriority: (bpId: string, eventId: string, direction: "up" | "down") => void;
  /** Drag-drop: move `fromId` relative to `toId`. Position controls before/after/inside (sub-event). */
  reorderEvent: (bpId: string, fromId: string, toId: string, position: "before" | "after" | "inside") => void;
  /**
   * Paste a previously-cloned event subtree into this BP. If `targetEventId`
   * is given, insert relative to that event (before / after / inside).
   * Otherwise append at the top of `pageId` (or first page if omitted).
   * The source is deep-cloned with fresh ids — safe to call multiple times
   * from the same clipboard. Returns the new root event id.
   */
  pasteEvent: (
    bpId: string,
    targetEventId: string | null,
    position: "before" | "after" | "inside" | "top",
    source: PeakyEvent,
    pageId?: string,
  ) => string;

  // Conditions live ON each event (replaces old guards + trigger split).
  addEventCondition: (bpId: string, eventId: string, kind: ConditionKind) => string;
  updateEventCondition: (bpId: string, eventId: string, condId: string, patch: Partial<Condition>) => void;
  removeEventCondition: (bpId: string, eventId: string, condId: string) => void;
  reorderEventCondition: (bpId: string, eventId: string, fromId: string, toId: string) => void;

  addEventAction: (bpId: string, eventId: string, kind: StateActionKind) => string;
  updateEventAction: (bpId: string, eventId: string, actionId: string, config: Record<string, unknown>) => void;
  /** Set the Construct-3-style subject on an action — what object the
   *  action operates against. Pass `undefined` to clear (action falls
   *  back to "self" on BP/widget sheets, "system" on Main Sheet). */
  setActionSubject: (bpId: string, eventId: string, actionId: string, subject: import("@peaky/shared").Subject | undefined) => void;
  removeEventAction: (bpId: string, eventId: string, actionId: string) => void;
  reorderEventAction: (bpId: string, eventId: string, fromActionId: string, toActionId: string) => void;

  // ---- event groups ----
  addEventGroup: (bpId: string, name?: string) => string;
  renameEventGroup: (bpId: string, groupId: string, name: string) => void;
  removeEventGroup: (bpId: string, groupId: string) => void;
  setEventGroupEnabled: (bpId: string, groupId: string, enabled: boolean) => void;
  assignEventToGroup: (bpId: string, eventId: string, groupId: string | null) => void;
  /** Add a new event page (Construct-style sheet tab). Returns the new page id. */
  addEventPage: (bpId: string, name?: string) => string;
  /** Rename an event page. */
  renameEventPage: (bpId: string, pageId: string, name: string) => void;
  /** Remove an event page. All events on that page are deleted with it. Cannot remove the last page. */
  removeEventPage: (bpId: string, pageId: string) => void;
  /** Reorder pages. Pass new page id order; ignored if length doesn't match. */
  reorderEventPages: (bpId: string, orderedIds: string[]) => void;
  /**
   * Wrap one or more existing top-level events in a new "group" event. The
   * selected events become the group's `children`, and the group takes their
   * position (at the first one's index). Same-page events only — out-of-page
   * ids are ignored. Returns the new group event id.
   */
  groupEvents: (bpId: string, eventIds: string[], name?: string) => string;
  /**
   * Assign multiple events to a group and pull them to be contiguous in the
   * priority order (placed at the position of the first selected event). If
   * `groupName` is given and no group with that name exists, create it first.
   */
  groupSelectedEvents: (bpId: string, eventIds: string[], groupName: string) => string;
...
export const useEditor = create<EditorState>((set, get) => ({
  ...
  // ─── Construct-3 Event Sheet ─────────────────────────────────────────
  addEvent: (bpId, parentId, kind, pageId) => {
    const id = newId("evt");
    set((state) => {
      // Resolve target page id for top-level events. Sub-events inherit
      // their parent's page implicitly so we don't write pageId on them.
      const bp = state.project.blueprints.find((b) => b.id === bpId);
      const targetPageId = parentId ? undefined : (pageId ?? bp?.eventPages[0]?.id);
      const fresh: PeakyEvent = {
        id, kind,
        conditions: [],
        actions: [],
        children: [],
        ...(kind === "comment" ? { text: "Comment" } : {}),
        ...(kind === "group" ? { text: "Group" } : {}),
        ...(targetPageId ? { pageId: targetPageId } : {}),
      };
      return {
        project: mapBlueprint(state.project, bpId, (b) => ({
          ...b,
          events: parentId
            ? mapEventTree(b.events, parentId, (ev) => ({ ...ev, children: [...ev.children, fresh] }))
            : [...b.events, fresh],
        })),
      };
    });
  ...
  setEventKind: (bpId, eventId, kind) =>
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => ({
        ...bp,
        events: mapEventTree(bp.events, eventId, (ev) => {
          const next: PeakyEvent = { ...ev, kind };
          // Switching to "comment" wipes conditions/actions/children.
          if (kind === "comment") {
            next.conditions = [];
            next.actions = [];
            next.children = [];
            if (!next.text) next.text = "Comment";
          }
          return next;
        }),
      })),
  ...
  setEventCombinator: (bpId, eventId, combinator) =>
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => ({
        ...bp,
        events: mapEventTree(bp.events, eventId, (ev) => ({ ...ev, combinator })),
      })),
    })),

  setEventCommentText: (bpId, eventId, text) =>
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => ({
        ...bp,
        events: mapEventTree(bp.events, eventId, (ev) => ({ ...ev, text })),
      })),
    })),

  setEventDisabled: (bpId, eventId, disabled) =>
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => ({
        ...bp,
        events: mapEventTree(bp.events, eventId, (ev) => ({ ...ev, disabled })),
      })),
    })),

  removeEvent: (bpId, eventId) => {
    // GC the localStorage collapse keys for this event AND every
    // descendant. Without this, deleted-event keys leak forever — a
    // long-lived project accumulates thousands of orphans, eventually
    // hitting the 5 MB origin cap.
    try {
      const proj = useEditor.getState().project;
      const bp = proj.blueprints.find((b) => b.id === bpId);
      const collect = (events: { id: string; children?: unknown[] }[] | undefined, target: string, ids: Set<string>, found: boolean): boolean => {
        if (!events) return found;
        for (const ev of events) {
          if (found || ev.id === target) {
            ids.add(ev.id);
            collect(ev.children as never, "", ids, true);
          } else {
            collect(ev.children as never, target, ids, false);
          }
        }
        return found;
      };
      const ids = new Set<string>();
      if (bp) collect(bp.events as never, eventId, ids, false);
      for (const id of ids) {
        try { localStorage.removeItem(`peaky.event.collapsed.${id}`); } catch { /* quota */ }
      }
    } catch { /* localStorage unavailable */ }
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => ({
        ...bp,
        events: filterEventTree(bp.events, eventId),
      })),
    }));
  },

  duplicateEvent: (bpId, eventId) => {
    const newRootId = newId("evt");
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => {
        const result = duplicateEventInTree(bp.events, eventId, newRootId);
  ...
  moveEventPriority: (bpId, eventId, direction) =>
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => ({
        ...bp,
        events: moveEventInTree(bp.events, eventId, direction),
      })),
    })),

  reorderEvent: (bpId, fromId, toId, position) =>
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => {
        if (fromId === toId) return bp;
        // Don't allow moving an event under one of its own descendants — that
        // would create a cycle. Find fromEvent's full subtree id set first.
        const blockedIds = new Set<string>();
        collectEventIds(bp.events, fromId, blockedIds);
        if (blockedIds.has(toId)) return bp;
        const { tree: removed, taken } = takeEventFromTree(bp.events, fromId);
        if (!taken) return bp;
        // Cross-page sanity: when dropping before/after a TOP-LEVEL event,
        // inherit its pageId so the moved event lives on the page where the
        // user dropped it. When dropping "inside" any event, clear pageId
        // since sub-events inherit page through their parent chain.
        const topLevelTarget = removed.find((e) => e.id === toId);
        let nodeToInsert: PeakyEvent = taken;
        if (position === "inside") {
          if (taken.pageId !== undefined) {
            nodeToInsert = { ...taken, pageId: undefined };
          }
        } else if (topLevelTarget && taken.pageId !== topLevelTarget.pageId) {
          nodeToInsert = { ...taken, pageId: topLevelTarget.pageId };
        }
        const reinserted = insertEventInTree(removed, toId, nodeToInsert, position);
        return { ...bp, events: reinserted };
      }),
    })),

  pasteEvent: (bpId, targetEventId, position, source, pageId) => {
    const cloned = cloneEventDeep(source);
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => {
        // Top-level append: assign to active page (or first page).
        if (position === "top" || !targetEventId) {
          const targetPage = pageId ?? bp.eventPages[0]?.id;
          const node = targetPage ? { ...cloned, pageId: targetPage } : cloned;
          return { ...bp, events: [...bp.events, node] };
        }
        // Inserted relative to a target event. Decide pageId based on where
        // it lands so cross-page paste doesn't strand the new node on the
        // source's old page.
        const topLevelTarget = bp.events.find((e) => e.id === targetEventId);
        let node: PeakyEvent;
        if (position === "inside") {
          // Becomes a sub-event — drop pageId so it inherits via parent.
          node = { ...cloned, pageId: undefined };
        } else if (topLevelTarget) {
          // Before/after a TOP-LEVEL event → inherit its page.
          node = { ...cloned, pageId: topLevelTarget.pageId ?? pageId };
        } else {
          // Target is itself a sub-event — pasted node will live in its
          // parent chain, so drop pageId.
          node = { ...cloned, pageId: undefined };
        }
        return { ...bp, events: insertEventInTree(bp.events, targetEventId, node, position) };
      }),
    }));
    return cloned.id;
  },
  ...
  // ─── Conditions ──────────────────────────────────────────────────────
  addEventCondition: (bpId, eventId, kind) => {
    const id = newId("cond");
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => ({
        ...bp,
        events: mapEventTree(bp.events, eventId, (ev) => {
          const fresh = defaultConditionFor(id, kind);
          // Else is mutually exclusive — it's the SOLE condition of an event.
          // Adding Else wipes any other conditions; adding anything else when
          // Else is present removes Else first.
          if (kind === "Else") {
            return { ...ev, conditions: [fresh] };
          }
          const hasElse = ev.conditions.some((c) => c.kind === "Else");
          const next = hasElse ? ev.conditions.filter((c) => c.kind !== "Else") : ev.conditions;
          return { ...ev, conditions: [...next, fresh] };
        }),
      })),
    }));
    return id;
  ...
  updateEventCondition: (bpId, eventId, condId, patch) =>
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => ({
        ...bp,
        events: mapEventTree(bp.events, eventId, (ev) => {
          const updated = ev.conditions.map((c) => (c.id === condId ? { ...c, ...patch } : c));
          // If this update changed a condition's kind to "Else", that condition
          // becomes the sole condition (Else mutual-exclusivity rule). If it
          // changed FROM Else to something else, no special action — the row
          // simply takes its new kind alongside any others (rare path).
          const target = updated.find((c) => c.id === condId);
          if (target?.kind === "Else") {
            return { ...ev, conditions: [target] };
          }
          return { ...ev, conditions: updated };
        }),
      })),
    })),

  removeEventCondition: (bpId, eventId, condId) =>
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => ({
        ...bp,
        events: mapEventTree(bp.events, eventId, (ev) => ({
  ...
  reorderEventCondition: (bpId, eventId, fromId, toId) =>
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => ({
        ...bp,
        events: mapEventTree(bp.events, eventId, (ev) => {
          const fromIdx = ev.conditions.findIndex((c) => c.id === fromId);
          const toIdx = ev.conditions.findIndex((c) => c.id === toId);
          if (fromIdx < 0 || toIdx < 0 || fromIdx === toIdx) return ev;
          const next = [...ev.conditions];
          const [m] = next.splice(fromIdx, 1);
          next.splice(toIdx, 0, m);
          return { ...ev, conditions: next };
        }),
      })),
    })),

  // ─── Actions ─────────────────────────────────────────────────────────
  addEventAction: (bpId, eventId, kind) => {
    const id = newId("act");
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => ({
        ...bp,
        events: mapEventTree(bp.events, eventId, (ev) => {
          const action: StateAction = { id, kind, config: { ...ACTION_DEFAULTS[kind] } };
          return { ...ev, actions: [...ev.actions, action] };
        }),
      })),
    }));
    return id;
  ...
  updateEventAction: (bpId, eventId, actionId, config) => {
    console.log("[updateEventAction]", { bpId, eventId, actionId, config });
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => ({
        ...bp,
        events: mapEventTree(bp.events, eventId, (ev) => ({
          ...ev,
          // Spread the incoming config so the stored object is OUR copy, not
          // a reference shared with the caller (e.g. the picker modal's
          // useState slot). Without this, two actions could end up pointing
          // at the same config object — display rows would render identically
          // even though the modal showed each correctly. Vite HMR can
          // reproduce this by preserving modal state across a hot reload.
          actions: ev.actions.map((a) => (a.id === actionId ? { ...a, config: { ...config } } : a)),
        })),
      })),
    }));
  },

  setActionSubject: (bpId, eventId, actionId, subject) =>
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => ({
        ...bp,
        events: mapEventTree(bp.events, eventId, (ev) => ({
          ...ev,
          actions: ev.actions.map((a) => (a.id === actionId ? { ...a, subject } : a)),
        })),
      })),
    })),

  removeEventAction: (bpId, eventId, actionId) =>
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => ({
        ...bp,
        events: mapEventTree(bp.events, eventId, (ev) => ({
  ...
  reorderEventAction: (bpId, eventId, fromActionId, toActionId) =>
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => ({
        ...bp,
        events: mapEventTree(bp.events, eventId, (ev) => {
          const fromIdx = ev.actions.findIndex((a) => a.id === fromActionId);
          const toIdx = ev.actions.findIndex((a) => a.id === toActionId);
          if (fromIdx < 0 || toIdx < 0 || fromIdx === toIdx) return ev;
          const next = [...ev.actions];
          const [moved] = next.splice(fromIdx, 1);
          next.splice(toIdx, 0, moved);
          return { ...ev, actions: next };
        }),
      })),
    })),

  // ----------------- Event Groups -----------------
  addEventGroup: (bpId, name) => {
    const id = newId("grp");
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => {
        const baseName = name ?? "New Group";
        let unique = baseName;
        let n = 2;
        while (bp.eventGroups.some((g) => g.name === unique)) unique = `${baseName} ${n++}`;
        return { ...bp, eventGroups: [...bp.eventGroups, { id, name: unique, enabled: true }] };
      }),
    }));
    return id;
  },

  renameEventGroup: (bpId, groupId, name) =>
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => {
        const target = bp.eventGroups.find((g) => g.id === groupId);
        if (!target) return bp;
        if (bp.eventGroups.some((g) => g.id !== groupId && g.name === name)) return bp; // collision
        const oldName = target.name;
        // Cascade-rename: SetEventGroupEnabled actions referencing the old name follow.
        const newEvents = transformAllEvents(bp.events, (ev) => ({
          ...ev,
          actions: ev.actions.map((a) =>
            a.kind === "SetEventGroupEnabled" && a.config.group === oldName
              ? { ...a, config: { ...a.config, group: name } }
              : a,
          ),
        }));
        return {
          ...bp,
          eventGroups: bp.eventGroups.map((g) => (g.id === groupId ? { ...g, name } : g)),
          events: newEvents,
        };
      }),
    })),

  removeEventGroup: (bpId, groupId) =>
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => ({
        ...bp,
        eventGroups: bp.eventGroups.filter((g) => g.id !== groupId),
        // Events in the deleted group become ungrouped — walk the tree.
        events: transformAllEvents(bp.events, (ev) =>
          ev.groupId === groupId ? { ...ev, groupId: undefined } : ev,
        ),
      })),
    })),

  // ----------------- Event Pages (Construct-style sheet tabs) -----------------
  addEventPage: (bpId, name) => {
    const id = newId("page");
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => {
        const baseName = name ?? `Page ${bp.eventPages.length + 1}`;
        let unique = baseName;
        let n = 2;
        while (bp.eventPages.some((p) => p.name === unique)) unique = `${baseName} ${n++}`;
        return { ...bp, eventPages: [...bp.eventPages, { id, name: unique }] };
      }),
    }));
    return id;
  },

  renameEventPage: (bpId, pageId, name) =>
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => {
        // Auto-suffix on collision so the rename always lands SOMEWHERE
        // visible — silent failure would confuse users typing a duplicate.
        let unique = name;
        let n = 2;
        while (bp.eventPages.some((p) => p.id !== pageId && p.name === unique)) {
          unique = `${name} ${n++}`;
        }
        return {
          ...bp,
          eventPages: bp.eventPages.map((p) => (p.id === pageId ? { ...p, name: unique } : p)),
        };
      }),
    })),

  removeEventPage: (bpId, pageId) =>
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => {
        if (bp.eventPages.length <= 1) return bp; // never delete the last page
        return {
          ...bp,
          eventPages: bp.eventPages.filter((p) => p.id !== pageId),
          // Events on the deleted page are removed (cascade including their sub-events).
          events: bp.events.filter((ev) => ev.pageId !== pageId),
        };
      }),
    })),

  reorderEventPages: (bpId, orderedIds) =>
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => {
        if (orderedIds.length !== bp.eventPages.length) return bp;
        const byId = new Map(bp.eventPages.map((p) => [p.id, p]));
        const reordered = orderedIds.map((id) => byId.get(id)).filter(Boolean) as typeof bp.eventPages;
        if (reordered.length !== bp.eventPages.length) return bp;
        return { ...bp, eventPages: reordered };
      }),
    })),

  groupEvents: (bpId, eventIds, name) => {
    const newGroupId = newId("evt");
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => {
        // Find the requested events at the TOP level (sub-events are skipped
        // — grouping nested rows would require restructuring trees, out of
        // scope for this op). Preserve original ordering.
        const idSet = new Set(eventIds);
        const picked: PeakyEvent[] = [];
        const indexes: number[] = [];
        bp.events.forEach((ev, i) => {
          if (idSet.has(ev.id)) { picked.push(ev); indexes.push(i); }
        });
        if (picked.length === 0) return bp;
        // All picked events must share a page so the group can take that page id.
        const groupPageId = picked[0].pageId;
        if (picked.some((p) => p.pageId !== groupPageId)) return bp;
        const insertAt = indexes[0];
        // Strip pageId from children (sub-events inherit via parent).
        const children: PeakyEvent[] = picked.map((p) => {
          const { pageId: _pid, ...rest } = p;
          void _pid;
          return rest;
        });
        const group: PeakyEvent = {
          id: newGroupId,
          kind: "group",
          conditions: [],
          actions: [],
          children,
          text: name ?? "Group",
          ...(groupPageId ? { pageId: groupPageId } : {}),
        };
        // Remove the picked events from their original positions, then insert
        // the group at the position of the first one.
        const remaining = bp.events.filter((ev) => !idSet.has(ev.id));
        const next: PeakyEvent[] = [
          ...remaining.slice(0, insertAt),
          group,
          ...remaining.slice(insertAt),
        ];
        return { ...bp, events: next };
      }),
    }));
    return newGroupId;
  },

  setEventGroupEnabled: (bpId, groupId, enabled) =>
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => ({
        ...bp,
        eventGroups: bp.eventGroups.map((g) => (g.id === groupId ? { ...g, enabled } : g)),
      })),
    })),

  assignEventToGroup: (bpId, eventId, groupId) =>
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => ({
        ...bp,
        events: mapEventTree(bp.events, eventId, (ev) => ({ ...ev, groupId: groupId ?? undefined })),
      })),
    })),

  groupSelectedEvents: (bpId, eventIds, groupName) => {
    let createdId = "";
    set((state) => ({
      project: mapBlueprint(state.project, bpId, (bp) => {
        // Find or create the group.
        let groupId = bp.eventGroups.find((g) => g.name === groupName)?.id;
        let nextGroups = bp.eventGroups;
        if (!groupId) {
          groupId = newId("grp");
          nextGroups = [...bp.eventGroups, { id: groupId, name: groupName, enabled: true }];
        }
        createdId = groupId;
        // Find the earliest selected event's index — that's where we'll cluster the group.
        const selectedSet = new Set(eventIds);
        const earliestIdx = bp.events.findIndex((e) => selectedSet.has(e.id));
        if (earliestIdx < 0) return { ...bp, eventGroups: nextGroups };
        // Pull selected events out (preserve their relative order), assign group, insert at earliestIdx.
        const selected = bp.events.filter((e) => selectedSet.has(e.id)).map((e) => ({ ...e, groupId: groupId! }));
        const remaining = bp.events.filter((e) => !selectedSet.has(e.id));
        // Insert selected at the position equivalent to the earliest selected event in the remaining list.
        // earliestIdx in original list — count how many of remaining come before it.
        let insertIdx = 0;
        for (let i = 0; i < earliestIdx; i++) {
          if (!selectedSet.has(bp.events[i].id)) insertIdx++;
        }
        const next = [...remaining.slice(0, insertIdx), ...selected, ...remaining.slice(insertIdx)];
        return { ...bp, events: next, eventGroups: nextGroups };
      }),
    }));
    return createdId;
  },
